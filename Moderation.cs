...
namespace JeanPascaline.Modules
{
    ...
    public class Moderation : InteractiveBase<SocketCommandContext>
    {
        ...
        [Command("clearprofile"), Alias("cp"), Summary("Supprime la description et les pronoms d'un profil. — Deletes summary and pronouns of a user's profile.")]
        [RequireUserPermission(GuildPermission.ManageMessages)]
        public async Task RemoveDescription(IGuildUser member)
        {
            await Context.Message.DeleteAsync();
            GuildAccount GuildData = GuildAccounts.GetAccount(Context.Guild);
            ...
            EmbedAuthorBuilder ClearProfileAuthor = Utilities.CreateAuthorEmbed(Utilities.GetAlert(GuildData, "CLEARPROFILECARDTITLE", member.ToString()), member.GetAvatarUrl());
            ...
            EmbedBuilder ClearProfileEmbed = Utilities.CreateEmbed(ClearProfileAuthor, Color.Green, Utilities.CreateListFields(FieldsData));

            await ReplyAndDeleteAsync(Utilities.GetAlert(GuildAccounts.GetAccount(Context.Guild), "CLEARPROFILE", member.Mention));
